        ...
        if 2 < len(message.text) < 100:
            ...
            if not files:
                ...
                await message.reply_photo(photo=random.choice(PICS),
                    caption=f"🎰စလော့ဂိမ်း, 🐬ငါးပစ်ဂိမ်း, ♠️ကာစီနို, ⚽️ဘောလုံး, ♣️ရှမ်းကိုးမီး, 🐯လေးကောင်ဂျင်နှင့် အခြားဂိမ်းပေါင်းများစွာကို Lotus999 မှာပိတ်ရက်မရှိဆော့နိုင်ပါပြီလူကြီးမင်းတိုခင်ဗျား 👏👏\nViber : 09763354949\nTelegram : @lotus999game"
                    f"\n\n🔎 Query : {message.text}"
                    f"\n🔮 Total Results : `{total_results}` - Series is Ready**🍁"
                    f"\n🙋🏼 Request by : {message.from_user.mention}\n\n</b><a href='https://t.me/MKSVIPLINK'>© MKS Channel</a>",
                    reply_markup=InlineKeyboardMarkup(
                        [[InlineKeyboardButton("📥  🅓🅞🅦🅝🅛🅞🅐🅓  📥", url=f'https://t.me/{temp.U_NAME}?start=pquery_{key}')]]
                    ))
...
async def next_page(bot, query: CallbackQuery):
    ...
    out =f"🎰စလော့ဂိမ်း, 🐬ငါးပစ်ဂိမ်း, ♠️ကာစီနို, ⚽️ဘောလုံး, ♣️ရှမ်းကိုးမီး, 🐯လေးကောင်ဂျင်နှင့် အခြားဂိမ်းပေါင်းများစွာကို Lotus999 မှာပိတ်ရက်မရှိဆော့နိုင်ပါပြီလူကြီးမင်းတိုခင်ဗျား 👏👏\nViber : 09763354949\nTelegram : @lotus999game\n\nHey {query.from_user.mention}\n\nမိတ်ဆွေရှာတာ **{search}** ကို မင်မင်ရှာတွေတာပြပေးထားပါတယ်။ ဇာတ်ကားနာမည်လေးကို နှိပ်လိုက်ပါ။ start လေးပြန်ပေါ်လာတာကို ထပ်နှိပ်ရင် Bot က Auto ပို့ပေးပါမယ်နော်။ \n\n"
    k = 1
    for file in files:
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton("⭕️ Click here to Donate to the my channel.⭕️", url="https://t.me/kpmovielist/277")]
        )
        btn.append(        
            [InlineKeyboardButton("👉🏻 𝐕𝐈𝐏 𝐒𝐞𝐫𝐢𝐞𝐬 𝐌𝐞𝐦𝐛𝐞𝐫 ဝင်ရန် 👌🏻", url="https://t.me/Kpautoreply_bot")]                          
        )       
        btn.append(
            [InlineKeyboardButton("⭕️ All Channel & Group Link ⭕️", url="https://t.me/Movie_Zone_KP/3")] 
        )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('♻️ JOIN OUR GROUP TO USE ME ♻️', url='https://t.me/MKS_REQUESTGROUP')
        ], [
            InlineKeyboardButton('⚜️VIP SERIES ', url='https://t.me/Kpautoreply_bot'),
            InlineKeyboardButton('🏷 Channel', url='https://t.me/MKSVIPLINK')
        ], [
...
async def manual_filters(client, message, text=False):
    group_id = message.chat.id
    name = text or message.text
    reply_id = message.reply_to_message.from_user.id if message.reply_to_message else message.message_id
                ...
                try:
                    if fileid == "None":
                        if btn == "[]":                                   
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                         [[InlineKeyboardButton("✅ 𝑪𝑳𝒊𝒄𝑲 𝑯𝑬𝑹𝑬 𝑻𝒐 𝑱𝑶𝑰𝑵 ✅", url=f'https://t.me/mksmainchannel')]]
                            )
                            )
                        else:
                            button = eval(btn)
                            ...
                            await message.reply_photo(photo=random.choice(PICS),
                                caption=f"🎰စလော့ဂိမ်း, 🐬ငါးပစ်ဂိမ်း, ♠️ကာစီနို, ⚽️ဘောလုံး, ♣️ရှမ်းကိုးမီး, 🐯လေးကောင်ဂျင်နှင့် အခြားဂိမ်းပေါင်းများစွာကို Lotus999 မှာပိတ်ရက်မရှိဆော့နိုင်ပါပြီလူကြီးမင်းတိုခင်ဗျား 👏👏\nViber : 09763354949\nTelegram : @lotus999game"
                                f"\n\n🔎 Query : {message.text} \n🙋 Request by :{message.from_user.mention}\n🔮 Results : {reply_text}</b><a href='https://t.me/Movie_Zone_KP/3'>© MKS & KP Channel</a>",
                                reply_markup=InlineKeyboardMarkup(button)                            
                            )
                    else:
                        if btn == "[]":
                            await message.reply_cached_media(
                                fileid,
                                caption=f"🎰စလော့ဂိမ်း, 🐬ငါးပစ်ဂိမ်း, ♠️ကာစီနို, ⚽️ဘောလုံး, ♣️ရှမ်းကိုးမီး, 🐯လေးကောင်ဂျင်နှင့် အခြားဂိမ်းပေါင်းများစွာကို Lotus999 မှာပိတ်ရက်မရှိဆော့နိုင်ပါပြီလူကြီးမင်းတိုခင်ဗျား 👏👏\nViber : 09763354949\nTelegram : @lotus999game"
                                f"\n\n🔎 Query : {message.text}\n🙋 Request by :{message.from_user.mention}\n🔮 Results : {reply_text}</b><a href='https://t.me/Movie_Zone_KP/3'>© MKS & KP Channel</a>" or "",
                                reply_markup=InlineKeyboardMarkup(
                                         [[InlineKeyboardButton("✅ 𝑪𝑳𝒊𝒄𝑲 𝑯𝑬𝑹𝑬 𝑻𝒐 𝑱𝑶𝑰𝑵 ✅", url=f'https://t.me/mksmainchannel')]]
                            )
                            )
